...
function take_place(chess_order,x,y)
{
    ...
    for(let i=0;i<player.chess[chess_order].num;i++)
    {
        ...
        {
            if(temp[0]>0)
                if(board[temp[0]-1][temp[1]]==="{{player_n}}")
                    return false;
            if(temp[0]<19)
                if(board[temp[0]+1][temp[1]]==={{player_n}})
                    return false;
            if(temp[1]>0)
                if(board[temp[0]][temp[1]-1]==={{player_n}})
                    return false;
            if(temp[1]<19)
                if(board[temp[0]][temp[1]+1]==={{player_n}})
                    return false;
    ...
    else
    {
        for (let i = 0; i < player.chess[chess_order].num; i++)
        {
            ...
            try
            {
                if (board[temp[0] - 1][temp[1] - 1] === {{player_n}} || board[temp[0] - 1][temp[1] + 1] === {{player_n}} || board[temp[0] + 1][temp[1] + 1] === {{player_n}} || board[temp[0] + 1][temp[1] - 1] === {{player_n}})
                    det = 1;
            }
            catch (err)
            {
